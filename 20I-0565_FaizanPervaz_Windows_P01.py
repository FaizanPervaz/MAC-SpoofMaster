...
def display_current_mac():
    try:
        output = subprocess.check_output(['getmac', '/NH', '/V']).decode('utf-8', 'ignore')
        mac_addresses = re.findall(r"([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})", output)
        mac_info = "\n".join([mac[0] + mac[1] for mac in mac_addresses])
        messagebox.showinfo("MAC Addresses", f"MAC Addresses:\n{mac_info}")
    except subprocess.CalledProcessError as e:
        messagebox.showerror("Error", f"An error occurred while trying to get MAC addresses: {e}")
    except Exception as e:
